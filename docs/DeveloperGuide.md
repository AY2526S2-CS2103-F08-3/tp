...
5. **Undo/redo edge case for `outlet unassign`**<br>
   Running `outlet unassign` on a candidate who is already unassigned currently behaves like a successful no-op, but undo/redo may fail because the previous outlet is `null`. This is a user-facing reliability issue: the command appears to succeed, but later history operations can break. A minimal fix is to handle the already-unassigned case explicitly (either return a dedicated message without mutating state, or guard undo/redo message formatting when previous outlet is absent).

6. **CSV parsing is not fully quote-aware**<br>
   The `addcsv` flow currently splits rows using commas directly, so quoted CSV conventions are not fully supported. This can cause edge-case parsing issues with files generated by spreadsheet tools (for example, quoted values with embedded commas or escaped quotes). A minimal mitigation is to document strict plain-CSV expectations; a more robust fix is to use a proper CSV parser library.

7. **Nearest-assignment fallback is non-deterministic**<br>
   When postal-coordinate matching cannot be resolved, nearest assignment falls back to random outlet selection. This can produce different results for the same input across runs, which may feel inconsistent to users. A minimal improvement is to use deterministic fallback (for example, first valid outlet by displayed order) while preserving current nearest-resolution logic.

8. **Outlet input length limits are currently strict**<br>
   Outlet fields currently enforce tight limits (`name` max 10 characters, `address` max 18 characters). These limits were introduced to keep the outlet list compact and reduce UI overflow risk in the current layout. However, they can be restrictive for realistic business names and addresses.

   As a future enhancement, these limits should be relaxed (or made configurable) together with corresponding UI adjustments so longer outlet values remain readable without breaking layout consistency.
